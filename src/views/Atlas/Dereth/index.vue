<template>
  <div id="dereth-atlas">
    ...
    <div
      id="sidebar-pane"
      class="bg-slate-200 dark:bg-slate-600 flex flex-col overflow-hidden"
    ...
    >
      ...
      <div class="p-4 flex flex-col overflow-hidden">
        ...
        <div><span @click="fetchSpawnMap" class="border border-slate-700 px-3 py-2 bg-slate-500 text-slate-50 rounded-lg inline">Spawn Map</span></div>
      </div>
    </div>
    <div id="map-pane" class="border-l border-slate-300 dark:border-slate-700">
      ...
      <div id="map">
        ...
        v-if="atlasStore.targetedPosition">
          ...
          <!-- <p>  {{ atlasStore.targetedPosition.coordinates.global }}</p> -->
          <p v-if="atlasStore.options.landblockGrid">{{ atlasStore.targetedPosition.objCellIdHex }}</p>

        
        </div>

        <div class="landblock-mode text-xs bg-slate-700 text-slate-300 rounded-lg font-bold px-3 py-2 border border-slate-900 drop-shadow-lg" v-if="atlasStore.options.landblockGrid">Landblock Mode</div>
      </div>
    </div>
  </div>
</template>
...
const fetchSpawnMap = async () => {
    ...
    try {
        ...
        const spawnData = await fetch(`http://localhost:${atlasStore.dbPort}/locations?wcid=7`);
