let locString =
  "Your location is: 0x3F300008 [2.475189 185.127151 -0.095000] 0.992630 0.000000 0.000000 0.121185";
...
export class Position {
  ...
  constructor(
    ...
    this.objCellIdHex =
      "0x" + Number(this.objCellId).toString(16).padStart(8, "0").toUpperCase();
  }
  ...
  static deserialize(locString) {
    if (!Position.isValidLocString(locString))
    ...
    const rotation = [matches[5], matches[6], matches[7], matches[8]].map(
  ...
  static fromRadar(radarString) {
    if (!Position.isValidRadar(radarString))
  ...
  static fromGlobal(globalX, globalY) {
    if (
      globalX > this.GLOBAL_COORDS_MAX ||
      globalX < this.GLOBAL_COORDS_MIN ||
      globalY > this.GLOBAL_COORDS_MAX ||
      globalY < this.GLOBAL_COORDS_MIN
    ) {
